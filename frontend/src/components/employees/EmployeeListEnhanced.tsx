...
export default function EmployeeListEnhanced() {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'salary', headerName: 'Salary', width: 120, renderCell: (params) => `$${params.value.toLocaleString()}` },
    { 
      field: 'isActive', 
      headerName: 'Status', 
      width: 100, 
  ...
  const onBulkCreate = async () => {
    try {
      ...
      const employees = lines.map(line => {
        const [firstName, lastName, email, departmentId, position, salary] = line.split(',');
        return {
      ...
      setSnackbar({ open: true, message: `${employees.length} employees created successfully`, severity: 'success' });
      setBulkData('');
      await load();
    } catch (error) {
      setSnackbar({ open: true, message: 'Error creating employees', severity: 'error' });
    }
  };
  ...
  const onExport = async () => {
    try {
      ...
      setSnackbar({ open: true, message: 'Error exporting employees', severity: 'error' });
    }
  };
  ...
  const stats = {
    ...
    avgSalary: rows.length > 0 ? rows.reduce((sum, emp) => sum + emp.salary, 0) / rows.length : 0,
    ...
    <Box>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card>
            <CardContent>
              <Typography color="textSecondary" gutterBottom>Avg Salary</Typography>
        ...
        <TabPanel value={tabValue} index={1}>
          <Box>
            ...
            <Typography variant="body2" color="textSecondary" sx={{ mb: 2 }}>
              Enter employee data in CSV format: FirstName, LastName, Email, DepartmentId, Position, Salary
            </Typography>
            <TextField
              multiline
              rows={10}
              ...
              sx={{ mb: 2 }}
            />
            <Button variant="contained" onClick={onBulkCreate} startIcon={<UploadIcon />}>
              Create Employees
            </Button>
          </Box>
        </TabPanel>
          ...
          <Box>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={6}>
                <Card>
                  <CardContent>
                    <Typography variant="h6" gutterBottom>Salary Ranges</Typography>
                    <Typography>Min: ${Math.min(...rows.map(emp => emp.salary)).toLocaleString()}</Typography>
                    <Typography>Max: ${Math.max(...rows.map(emp => emp.salary)).toLocaleString()}</Typography>
                    <Typography>Average: ${stats.avgSalary.toLocaleString()}</Typography>
                  </CardContent>
                </Card>
              </Grid>
            </Grid>
